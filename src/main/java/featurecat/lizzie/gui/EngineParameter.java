...
public class EngineParameter extends JDialog {
  ...
  /** Create the dialog. */
  public EngineParameter(String enginePath, String weightPath, String helpCommand) {
    ...
    if (enginePath.toLowerCase().contains("main"))
      txtCommandLine.setText(enginePath + " -model " + weightPath);
    else txtCommandLine.setText(enginePath + " --weights " + weightPath);
    ...
    if (enginePath.toLowerCase().contains("main"))
      txtParameter.setText(" gtp -config gtp_example.cfg");
    else txtParameter.setText("-g --lagbuffer 0 ");
