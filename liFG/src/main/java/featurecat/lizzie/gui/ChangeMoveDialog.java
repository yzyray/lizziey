...
public class ChangeMoveDialog extends JDialog {
	...
	private boolean checkMove() {
		...
		if (changeMoveNumber <= 0 || changeMoveNumber > Lizzie.board.getHistory().getEnd().moveNumberOfNode()) {
			txtMoveNumber.setToolTipText(resourceBundle.getString("LizzieChangeMove.txtMoveNumber.error"));
			Action action = txtMoveNumber.getActionMap().get("postTip");
			if (action != null) {
				ActionEvent ae = new ActionEvent(txtMoveNumber, ActionEvent.ACTION_PERFORMED, "postTip",
		...
		if ("pass".equals(changePosition) || "swap".equals(changePosition)
				|| (changeCoord.isPresent() && Board.isValid(changeCoord.get()))) {
			txtChangeCoord.setToolTipText("");
			txtChangeCoord.setBackground(c);
		} else {
			txtChangeCoord.setToolTipText(resourceBundle.getString("LizzieChangeMove.txtChangeCoord.error"));
			txtChangeCoord.setBackground(Color.red);
			Action action = txtChangeCoord.getActionMap().get("postTip");
			if (action != null) {
				ActionEvent ae = new ActionEvent(txtChangeCoord, ActionEvent.ACTION_PERFORMED, "postTip",
						EventQueue.getMostRecentEventTime(), 0);
				action.actionPerformed(ae);
			}
			ret = false;
